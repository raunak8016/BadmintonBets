...
def getRankings(url):
  ...
  #  driver.find_element(By.XPATH, '/html/body/div/div/div/main/form/div[1]/button[1]/span').click()
    ...
    df = pd.read_csv(r'C:\Users\rauna\BadmintonApp\Data_Handling\rankings.csv')
    ...
    for i in range(1, 20):
        ...
        for row in rows:
            ...
            date_obj = time.strptime(ranking_week, "%m/%d/%Y")
            formated_date = time.strftime("%m/%d/%Y", date_obj)
            new_row = pd.DataFrame([[formated_date, ranking, name.upper(), country, points, tourn, member_id]], columns=['Ranking Week', 'Rank', 'Name', 'Country', 'Points', 'Tournaments', 'Member ID'])
           # print(new_row)
            df = pd.concat([df, new_row], ignore_index=True)

    return df
df = getRankings(url)
df.to_csv(r'C:\Users\rauna\BadmintonApp\Data_Handling\rankings.csv', index=False)
